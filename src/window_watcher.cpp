...
WindowWatcher::WindowWatcher(uint8_t window_number, QWidget *parent) : QWidget(parent)
{
    // Create each window
    ...
    switch(window_number) {
        ...
        case 3:
            ...
            mainWindow->setMinimumSize(850, 500);
...
void WindowWatcher::advanceWindow()
{
	switch(currentWindow) {
		...
		case windows::SECOND_WINDOW:
            qInfo() << "case second window, should open main window";
            firstWindow->hide();
            secondWindow->hide();
            mainWindow->setMinimumSize(850, 500);
	        mainWindow->showMaximized();
